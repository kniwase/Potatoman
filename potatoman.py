...
def fullgame(playerName, AI_level, isSilent = False):
	...
	#CPUプレイヤーの作成
	AI_level_mod = [AI if AI != 'H' else -1 for AI in AI_level]
	players = [potatomanCore.Player(playerName[i], hand[i], potatomanAI.level[AI_level_mod[i]], AI_level_mod[i] == -1) for i in range(4)]
	#プレイヤーのシャッフル
	...
	while flg[0] == 0:
		...
		flg, result_str = potatomanCore.trick(players, isSilent)
		#ターンの結果表示
		if not isSilent: print()
		...
		for player, color in zip(players, potatomanCore.colors):
			if not isSilent: print('%-14s%-10s' % ('{0:<4} : {1:>2}点'.format(player.getName(), player.getPoint()), '{0} :  {1}枚'.format(potatomanCore.colors_str[color], pointCard[color])))
		if not isSilent: print()
		count = count + 1
		if flg[1] != 0:
